...
export default function DashboardPage() {
  ...
  const statsEn = [
    { label: "Enrolled Courses", value: "02", icon: BookOpen, color: "text-blue-600", bg: "bg-blue-50" },
    { label: "Completed Lessons", value: "15", icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Certificates", value: "01", icon: Award, color: "text-amber-600", bg: "bg-amber-50" },
    { label: "Study Hours", value: "24", icon: Clock, color: "text-purple-600", bg: "bg-purple-50" },
  ];
  ...
  const activitiesEn = [
    { title: "Completed 'Neurophysiology' lesson", time: "2 hours ago", icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-50" },
    { title: "Scored 9/10 on Anatomy quiz", time: "Yesterday", icon: Star, color: "text-amber-500", bg: "bg-amber-50" },
    { title: "Started 'Clinical Biochemistry' course", time: "2 days ago", icon: PlayCircle, color: "text-blue-500", bg: "bg-blue-50" },
  ];

  const activities = isArabic ? activitiesAr : activitiesEn;
    ...
    <div className="flex flex-col min-h-screen font-sans bg-slate-50 overflow-x-hidden selection:bg-blue-100 selection:text-blue-900">
      ...
      <main className="flex-grow pt-32 pb-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Welcome Header */}
          <div className={`flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700 ${isArabic ? 'text-right' : 'text-left'}`}>
            <div className="w-full lg:w-auto">
              <Badge className="mb-4 bg-blue-100 text-blue-700 border-none px-4 py-1 font-bold rounded-full">
              ...
              <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight mb-2">
                {isArabic ? (
                  ...
                  <>Welcome, <span className="text-blue-700">Ahmed Khaled</span> 👋</>
                )}
              </h1>
              <p className="text-lg text-slate-500 font-medium leading-relaxed">
                {isArabic 
                  ? "استكمل رحلتك التعليمية وتابع تقدمك في الكورسات الطبية."
                  : "Continue your learning journey and track your progress in medical courses."
                }
              </p>
            </div>
            <div className="flex gap-4 w-full lg:w-auto">
               <Link href="/courses" className="flex-1 lg:flex-none">
                 <Button className="w-full bg-blue-700 hover:bg-blue-800 text-white px-8 h-14 text-lg font-black rounded-2xl shadow-lg shadow-blue-200 active:scale-95 transition-all">
                    {isArabic ? "استكشف كورسات جديدة" : "Explore New Courses"}
                 </Button>
               </Link>
            </div>
          </div>
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl font-black text-slate-900">{isArabic ? "كورساتي الحالية" : "My Current Courses"}</h2>
                  <Link href="/courses" className="text-sm font-bold text-blue-700 hover:underline">{isArabic ? "عرض الكل" : "View All"}</Link>
                </div>
                  ...
                  {enrolledCourses.map((course) => (
                    <Card key={course.id} className="overflow-hidden border-none shadow-sm hover:shadow-xl transition-all duration-500 bg-white rounded-[2.5rem] group">
                      <div className="h-44 relative">
                        ...
                        <Badge className={`absolute top-4 ${isArabic ? 'right-4' : 'left-4'} bg-white/95 text-blue-700 border-none font-bold`}>
                           {course.category}
                        </Badge>
                      </div>
                      <CardHeader className={`pt-6 px-8 ${isArabic ? 'text-right' : 'text-left'}`}>
                      ...
                      <CardContent className="px-8 pb-8">
                        <div className="flex items-center justify-between mb-2">
                           <span className="text-xs font-bold text-slate-400 uppercase">{isArabic ? "التقدم:" : "Progress:"} 65%</span>
                           <span className="text-xs font-bold text-blue-700">12/18 {isArabic ? "درس" : "lessons"}</span>
                        </div>
              ...
              {/* Recent Activity */}
              <div className={`bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm ${isArabic ? 'text-right' : 'text-left'}`}>
                <h2 className="text-2xl font-black text-slate-900 mb-8">{isArabic ? "آخر النشاطات" : "Recent Activity"}</h2>
                <div className="space-y-6">
                  {activities.map((item, i) => (
                    <div key={i} className={`flex items-start gap-4 p-4 rounded-2xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100 ${isArabic ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`space-y-8 ${isArabic ? 'text-right' : 'text-left'}`}>
              ...
              <div className="bg-blue-900 text-white p-10 rounded-[3rem] shadow-2xl relative overflow-hidden group">
                 <div className="relative z-10 text-center">
                    ...
                    <h3 className="text-2xl font-black mb-1 text-center">{isArabic ? "أحمد خالد" : "Ahmed Khaled"}</h3>
                    <p className="text-blue-300 text-sm font-bold uppercase tracking-widest mb-6 text-center">
                      {isArabic ? "طالب طب - السنة الثالثة" : "Medical Student - Year 3"}
                    </p>
                    <div className="w-full h-[1px] bg-white/10 mb-6"></div>
                    <Button className="w-full bg-white/10 border-2 border-white/20 text-white hover:bg-white/20 rounded-xl h-12 font-bold transition-all backdrop-blur-sm">
                      {isArabic ? "تعديل الملف الشخصي" : "Edit Profile"}
                    </Button>
                 </div>
                 {/* Abstract Decor */}
                 <div className={`absolute -top-10 ${isArabic ? '-left-10' : '-right-10'} w-40 h-40 bg-white/5 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-1000`}></div>
              </div>

              {/* Recommended for you */}
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
                <h2 className={`text-xl font-black text-slate-900 mb-6 flex items-center gap-2 ${isArabic ? 'justify-end' : 'justify-start'}`}>
                   <Sparkles size={20} className="text-blue-700" />
                   {isArabic ? "موصى به لك" : "Recommended for You"}
                </h2>
                <div className="space-y-4">
                  {courses.slice(2, 4).map((course) => (
                    <Link href={`/courses/${course.slug}`} key={course.id} className={`flex items-center gap-4 p-3 hover:bg-slate-50 rounded-2xl transition-all border border-transparent hover:border-slate-100 group ${isArabic ? 'flex-row-reverse' : ''}`}>
                ...
                <Link href="/courses">
                  <Button variant="link" className={`w-full mt-6 text-blue-700 font-black flex items-center justify-center gap-2 hover:no-underline ${isArabic ? 'flex-row-reverse' : ''}`}>
                     {isArabic ? <ArrowRight size={18} className="rotate-180" /> : <ArrowLeft size={18} className="rotate-180" />}
                     {isArabic ? "استعرض الكتالوج الكامل" : "Browse Full Catalog"}
                  </Button>
                </Link>
              </div>
              ...
              <div className="bg-emerald-50/50 p-8 rounded-[3rem] border border-emerald-100">
                <h2 className="text-xl font-black text-slate-900 mb-4">{isArabic ? "هل تحتاج مساعدة؟" : "Need Help?"}</h2>
                ...
                <Link href="/contact">
                  <Button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl h-12 shadow-lg shadow-emerald-100">
                     {isArabic ? "تحدث مع الدعم" : "Contact Support"}
                  </Button>
                </Link>
              </div>
